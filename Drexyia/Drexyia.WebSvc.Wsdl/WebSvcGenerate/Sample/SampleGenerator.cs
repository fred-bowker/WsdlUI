...
namespace Drexyia.WebSvc.Wsdl.Sample {
    public class SampleGenerator {
        ...
        void WriteElementSample(XmlTextWriter xtw, string ns, XmlSchemaElement elem) {
            ...
            //TODO: FB this is a fix for dealing with the InvalidCastException 
            //test data for this bug is C:\Work\NotUnderSource\WsdlUI_v1\test_data\failed\InvalidCastException\create_user_net.wsdl Node name="NotificationInd"
            else if (elem.SchemaTypeName.IsEmpty && elem.SchemaType is XmlSchemaSimpleType) {
                xtw.WriteStartElement(root.Name, root.Namespace);
                xtw.WriteString("byte");
                xtw.WriteEndElement();
            }
            else
        ...
        string GetAbsolutePath(string uri) {
            string uriRegex = @"http:\/\/([A-Za-z0-9]+):[A-Za-z0-9]+(.+)";
            Match regexMatch = Regex.Match(uri, uriRegex);

            string absolutePath = regexMatch.Groups[2].Value;
            return absolutePath;
        }
